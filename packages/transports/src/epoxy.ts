...
import initEpoxy, { EpoxyClient, EpoxyClientOptions, EpoxyHandlers, info as epoxyInfo } from "@mercuryworkshop/epoxy-tls/epoxy";

// slightly modified verison of https://github.com/MercuryWorkshop/epoxy-transport/
...
export default class EpoxyTransport implements ProxyTransport {
    ...
    setopt(opts: EpoxyClientOptions, opt: string) {
        const value = this.opts[opt as keyof EpoxyOptions];
        if (value != null)
            (opts as unknown as Record<string, unknown>)[opt] = value;
    }

    async init() {
        await initEpoxy({ module_or_path: this.wasm });
    ...
    connect(
        ...
        return [
            ...
            async (code, reason) => (await ws).close(code, reason || ""),
        ];
    }
}