...
use bk_types::resources::VanguardResource;
use js_sys::Object;
...
pub struct VanguardInlineWebAcessibleResources(HashMap<String, Vec<u8>>);

#[wasm_bindgen]
impl VanguardInlineWebAcessibleResources {

    #[wasm_bindgen(constructor)]
    // Amazingly, serdews can't convert a js map to a rust hashmap, but can do the reverse
    pub fn new(web_accessible_resources: &JsValue) -> VanguardInlineWebAcessibleResources {
        VanguardInlineWebAcessibleResources(serdews::from_value(web_accessible_resources.into()).unwrap())
    }
...
pub struct VanguardResourceAssemblerInfo(String, VanguardInlineWebAcessibleResources, Option<String>);

#[wasm_bindgen]
impl VanguardResourceAssemblerInfo {

    #[wasm_bindgen(constructor)]
    pub fn new(
        redirect_resources_content: String,
        web_accessible_resources: VanguardInlineWebAcessibleResources,
        scriplet_js_content: Option<String>
    ) -> VanguardResourceAssemblerInfo {
        VanguardResourceAssemblerInfo(redirect_resources_content, web_accessible_resources, scriplet_js_content)
    }
...
#[wasm_bindgen]
#[derive(Clone)]
pub struct VanguardAssembledResources(Vec<Resource>);

#[wasm_bindgen]
impl VanguardAssembledResources {

    #[wasm_bindgen(constructor)]
    pub fn new(resources: Vec<VanguardResource>) -> Self {
        Self( unsafe { std::mem::transmute(resources) })
    }

    #[wasm_bindgen(js_name = toArray)]
    pub fn to_array(&self) -> Vec<VanguardResource> {
...
fn read_resource_from_web_accessible_dir(
    inline_webad: &VanguardInlineWebAcessibleResources,
...
pub fn assemble_resources(
    resource_assembler_info: &VanguardResourceAssemblerInfo
) -> Result<VanguardAssembledResources, JsError> {
    ...
    Ok(VanguardAssembledResources(web_accessible_resources.map(|mut ress| {
...
#[wasm_bindgen]
#[derive(Clone)]
pub struct VanguardExclusionStore(Vec<WildMatch>);

#[wasm_bindgen]
impl VanguardExclusionStore {

    #[wasm_bindgen(constructor)]
    pub fn new(hosts: Vec<String>) -> Self {
        VanguardExclusionStore(
            hosts.into_iter()
