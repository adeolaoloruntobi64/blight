...
static TOP_COMMENT_RE: LazyLock<Regex> = LazyLock::new(|| Regex::new(r#"^/\*[\S\s]+?\n\*/\s*"#).unwrap());
...
static UNQUOTED_FIELD_RE: LazyLock<Regex> = LazyLock::new(|| Regex::new(r#"([\{,])([a-zA-Z][a-zA-Z0-9_]*):"#).unwrap());
// Avoid matching a starting `/*` inside a string
static TRAILING_BLOCK_COMMENT_RE: LazyLock<Regex> = LazyLock::new(|| Regex::new(r#"\s*/\*[^'"]*\*/\s*$"#).unwrap());
    ...
    for line in uncommented.lines() {
        ...
        resources.push(Resource {
            ...
            aliases: details
                ...
                .unwrap_or_default(),
            kind,
            content: STANDARD.encode(&script),
            dependencies: vec![],
...
pub fn build_resource_from_file_contents(
    ...
    let content = match mimetype {
        MimeType::ApplicationJavascript | MimeType::TextHtml | MimeType::TextPlain => {
            ...
            STANDARD.encode(&utf8string.replace('\r', ""))
        }
        _ => STANDARD.encode(&resource_contents),
    };
...
pub fn read_resource_from_web_accessible_dir(
