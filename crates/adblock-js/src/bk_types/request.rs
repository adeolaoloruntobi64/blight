...
pub struct AdBlockRequest {
    pub request_type: AdBlockRequestType,
...
impl AdBlockRequest {
    ...
    pub fn new(url: &str, source_url: &str, request_type: &str) -> Result<AdBlockRequest, RequestError> {
        unsafe {
    ...
    pub fn preparsed(
        ...
        unsafe { 
            std::mem::transmute(
                Request::preparsed(url, hostname, source_hostname, request_type, third_party)
            )
        }
    }
}
